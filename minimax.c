...
ZUSTAND _minimax(ZUSTAND z, int tiefe, int player, nachfolgezustandsfunktion nzf, zustandsbewertungsfunktion zbf) {
    ...
    for (int i = 0; i < list_size(nachfolgezustaende); i++) {
        ...
        int bewertung = 0;
        if (tiefe > 0) {
            bester_nachfolgezustand = minimax(aktueller_nachfolgezustand, tiefe - 1, -player, nzf, zbf);
        } else {
...
ZUSTAND minimax(ZUSTAND z, int tiefe, int player, nachfolgezustandsfunktion nzf, zustandsbewertungsfunktion zbf) {
    _minimax(z, tiefe, player, nzf, zbf);
}